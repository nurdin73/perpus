...
class Mod_laporan extends CI_Model {
     ...
     public function cetak($start, $end)
    {
        ...
        return $this->db->query("SELECT a.*,  
                                 ...
                                 ELSE 'Sudah Dikembalikan' 
                                 END AS status_pinjam
                                 FROM transaksi a WHERE a.tanggal_pinjam  BETWEEN '$start' AND '$end' GROUP BY a.id_transaksi");
    }   
    ...
    public function cetakPengembalian($start, $end)
    {
        return $this->db->query("SELECT a.*, b.id_petugas, b.full_name FROM pengembalian a, petugas b WHERE                             a.tgl_pengembalian BETWEEN '$start' AND '$end'
                                 AND a.id_petugas = b.id_petugas 
                                 GROUP BY a.id_transaksi");
    }
